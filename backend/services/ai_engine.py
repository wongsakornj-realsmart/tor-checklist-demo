...
Architecture:
  ...
  10. Final Polish (dynamically populates highly accurate document types and detailed instructions matching sample TOR checklists)
"""
import os
import json
import re
...
def generate_tor_checklist(text_content: str) -> dict:
    ...
    if not all_items:
        ...
        valid_lines = [l for l in lines if len(re.findall(r'[\u0E00-\u0E7F]', l)) >= 5 and not any(kw in l for kw in ['กรรมการ (', 'ลงนาม...', 'ครั้งที่...', 'สารบัญ', 'TOR 1', 'เอกสารแนบ'])]
    ...
    # Step 8: Final Polish - Ensure no empty values and enforce deep diversity in document types & instructions matching ground truth
    ...
    for item in all_items:
        ...
        if not subcat or subcat in ['None', 'รายละเอียดข้อกำหนด', '']:
            if any(w in req for w in ['ฮาร์ดแวร์', 'เครื่อง', 'อุปกรณ์', 'เซิร์ฟเวอร์', 'Hardware', 'Server', 'สเปก']):
                item['หัวข้อย่อย'] = "คุณลักษณะเฉพาะของอุปกรณ์ฮาร์ดแวร์"
            elif any(w in req for w in ['ซอฟต์แวร์', 'ระบบ', 'ฟังก์ชัน', 'Software', 'System', 'แอปพลิเคชัน', 'หน้าจอ']):
                item['หัวข้อย่อย'] = "ข้อกำหนดทางเทคนิคและฟังก์ชันการทำงานของระบบ"
            elif any(w in req for w in ['ผลงาน', 'ประสบการณ์', 'เคยทำงาน', 'คุณสมบัติ', 'นิติบุคคล', 'ทุนจดทะเบียน']):
                item['หัวข้อย่อย'] = "คุณสมบัติของผู้ยื่นข้อเสนอ"
            elif any(w in req for w in ['ส่งมอบ', 'งวดงาน', 'กำหนดเวลา', 'งวดที่', 'ตรวจรับ']):
                item['หัวข้อย่อย'] = "เงื่อนไขการส่งมอบงานและการตรวจรับ"
            elif any(w in req for w in ['รับประกัน', 'บำรุงรักษา', 'SLA', 'แก้ไขปัญหา', 'ซ่อมแซม', 'Maintenance']):
                item['หัวข้อย่อย'] = "การรับประกันระบบและการบำรุงรักษา"
            elif any(w in req for w in ['อบรม', 'ฝึกอบรม', 'ถ่ายทอดเทคโนโลยี', 'คู่มือ', 'Training']):
                item['หัวข้อย่อย'] = "การฝึกอบรมและถ่ายทอดเทคโนโลยี"
            elif any(w in req for w in ['บุคลากร', 'ทีมงาน', 'ผู้จัดการ', 'ผู้เชี่ยวชาญ', 'วุฒิการศึกษา']):
        ...
        if not doc or doc == 'None' or doc == 'ข้อเสนอทางเทคนิค (Technical Proposal)':
            ...
            elif any(w in req for w in ['ใบอนุญาต', 'หนังสือแต่งตั้ง', 'ตัวแทนจำหน่าย', 'MAF', 'ลิขสิทธิ์']):
            ...
            elif any(w in req for w in ['ใบเสนอราคา', 'ราคา', 'งบประมาณ', 'หลักประกัน', 'BOQ']):
                item['ชื่อเอกสารที่ใช้ยื่น'] = "ใบเสนอราคา (ในระบบ e-GP) และบัญชีรายการแสดงราคา (Bill of Quantities: BOQ)"
            elif any(w in req for w in ['ตารางเปรียบเทียบ', 'แคตตาล็อก', 'Catalog', 'Specification', 'ยี่ห้อ', 'สเปก', 'ฮาร์ดแวร์']):
