...
# Common Thai words that must appear in readable text (not random byte sequences)
THAI_READABLE_WORDS = [
    "การ", "ของ", "และ", "ใน", "ที่", "ให้", "ต้อง", "เป็น", "มี", "ได้",
    "จะ", "หรือ", "โดย", "ไม่", "กับ", "จาก", "เพื่อ", "ระบบ", "งาน", "โครงการ",
    "ผู้", "แล้ว", "ตาม", "อย่าง", "ไป", "มา", "คือ", "นั้น", "จัด", "ทำ",
    "สามารถ", "ข้อ", "รับ", "ส่ง", "พัฒนา", "บริการ", "ข้อมูล", "เอกสาร",
    "หน่วยงาน", "สัญญา", "ราคา", "คุณสมบัติ", "ขอบเขต", "กำหนด",
    "ดำเนินงาน", "วัตถุประสงค์", "ความ", "เงื่อนไข", "หลัก", "ประกัน"
]


def _is_readable_thai(text: str) -> bool:
    """
    Checks if a string contains actual readable Thai text (not random byte garbage).
    Real Thai text has common words like การ, ของ, และ, ระบบ etc.
    Binary garbage (e.g. ๐๋ฟ๋๔ฟ๔ฟฏฬ) contains Thai characters but no real words.
    """
    if not text or len(text) < 5:
        return False
    word_count = sum(1 for w in THAI_READABLE_WORDS if w in text)
    # A real Thai sentence of >=10 chars should contain at least 1 common word
    # A paragraph should contain at least 2
    if len(text) > 30:
        return word_count >= 2
    return word_count >= 1
...
def _safe_print(msg: str):
    """Safely prints messages to console without throwing fatal UnicodeEncodeErrors in Windows PowerShell."""
    try:
        print(msg)
    except UnicodeEncodeError:
        try:
            print(msg.encode('cp874', 'replace').decode('cp874', 'ignore'))
        except:
            pass
...
def _setup_tesseract():
    ...
    try:
        if not os.path.exists(tessdata_path_tha):
            _safe_print("[OCR Service] Downloading tha.traineddata...")
    ...
    # Linux paths (Render cloud server via Aptfile)
    elif os.path.exists('/usr/bin/tesseract'):
        pytesseract.pytesseract.tesseract_cmd = '/usr/bin/tesseract'
        # On Linux with apt-installed tesseract, use system tessdata
        linux_tessdata = '/usr/share/tesseract-ocr/5/tessdata'
        if not os.path.exists(linux_tessdata):
        ...
        if os.path.exists(linux_tessdata):
            ...
            _safe_print(f"[OCR Service] Using Linux system tessdata at {linux_tessdata}")
            return True
    ...
    for line in text.split('\n'):
        ...
        if any(kw in l_strip for kw in ['กรรมการ (', 'ลงนาม...', 'ครั้งที่...', 'ประธานกรรมการ', 'คณะกรรมการกำหนดขอบเขต', 'ผู้ตรวจทาน', 'ผู้ร่าง', 'กรรมการและเลขานุการ']):
            if any(sw in l_strip for sw in ['นาย', 'นาง', 'ลงนาม', 'ครั้งที่', 'กรรมการ', 'นางสาว']):
                _safe_print(f"[OCR Service] Filtered committee signature footer: {l_strip[:30]}")
                continue
...
def extract_text_from_file(file_path: str) -> str:
    ...
    CRITICAL FIX: Completely eliminates toxic deep binary salvage. If pdfplumber
    returns empty/corrupted text, forces pristine visual OCR via pytesseract.
    """
    ...
    try:
        if ext == '.pdf':
            try:
                ...
                _safe_print(f"pdfplumber text extraction error: {pdf_err}")
            ...
            # If thai_count < 50 OR keyword_matches < 3, the text is empty/corrupted! Force Optical Character Recognition (pytesseract)!
            if thai_count < 50 or keyword_matches < 3:
                _safe_print(f"[OCR Service] Extracted text appears empty/corrupted (Thai chars={thai_count}, Valid Keywords={keyword_matches}). Forcing pristine visual OCR via pytesseract...")
                ocr_text = ""
                if _setup_tesseract():
                    try:
                        with pdfplumber.open(file_path) as pdf:
                            for i, page in enumerate(pdf.pages):
                                _safe_print(f"[OCR Service] Rendering and OCRing PDF page {i+1}...")
                                img = page.to_image(resolution=150).original
                                ocr_text += pytesseract.image_to_string(img, config='-l tha+eng') + "\n"
                    except Exception as ocr_err:
                        _safe_print(f"[OCR Service] pytesseract OCR error: {ocr_err}")
                
                ocr_text = _clean_and_filter_text(ocr_text)
                ocr_thai_count = len(re.findall(r'[\u0E00-\u0E7F]', ocr_text))
                
                if ocr_thai_count > thai_count:
                    _safe_print(f"[OCR Service] pytesseract visual OCR successful (Thai chars={ocr_thai_count}). Replacing corrupted text with pristine OCR text.")
                    full_text = ocr_text
                else:
                    _safe_print("[OCR Service] pytesseract unavailable or yielded no text. Returning clean notice instead of binary gibberish.")
            ...
            if _setup_tesseract():
                try:
                    ...
                    _safe_print(f"OCR Error on Image: {ocr_err}")
        ...
        _safe_print(f"Extraction error for {file_path}: {e}")
